...
if __name__ == "__main__":
    ...
    if config.get("bar_start_hidden", False):
        def hide_shell_components():
            for monitor in monitors:
                monitor_id = monitor["id"]
                if multi_monitor_enabled and monitor_manager:
                    bar_instance = monitor_manager.get_instance(monitor_id, "bar")
                    notch_instance = monitor_manager.get_instance(monitor_id, "notch")
                    dock_instance = monitor_manager.get_instance(monitor_id, "dock")
                else:
                    bar_instance, notch_instance, dock_instance = bar, notch, dock

                if bar_instance:
                    bar_instance.set_visible(False)
                if notch_instance:
                    notch_instance.force_occlusion()
                if dock_instance:
                    dock_instance.force_occlusion()

            return False

        GLib.idle_add(hide_shell_components)
