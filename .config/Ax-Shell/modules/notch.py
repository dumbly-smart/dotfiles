...
class Notch(Window):
    def __init__(self, monitor_id: int = 0, **kwargs):
        ...
        self._current_window_identity = self._get_current_window_identity()
    ...
    def _get_current_window_identity(self):
        """Get a stable identity for the currently active window."""
        try:
            from fabric.hyprland.widgets import get_hyprland_connection
            ...
            if conn:
                ...
                active_window_json = conn.send_command("j/activewindow").reply.decode()
                active_window_data = json.loads(active_window_json)
                return active_window_data.get("address", "")
        except Exception as e:
    ...
    def on_active_window_changed(self, *args):
        ...
        new_window_identity = self._get_current_window_identity()

        if new_window_identity != self._current_window_identity:
            self._current_window_identity = new_window_identity
            ...
            self._occlusion_timer_id = GLib.timeout_add(
                1500, self._restore_occlusion_check
            )
    ...
    def on_key_press(self, widget, event):
        ...
        if (
            ...
            # Let focused text inputs handle typing themselves.  Without this,
            # entering a Wi-Fi password triggers the dashboard's type-to-launch
            # shortcut and replaces the network applet with the app launcher.
