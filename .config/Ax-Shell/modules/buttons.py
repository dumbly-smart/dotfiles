...
class NetworkButton(Box):
    ...
    def _toggle_wifi(self, *_):
        ...
        self.network_ssid.set_label(
            "Turning on…" if target else "Turning off…"
        )
    ...
    def _confirm_wifi_power(self, target, retry):
        ...
        if not wifi:
            return False
        # Ignore an older timer when the user has already requested the opposite state.
...
class BluetoothButton(Box):
    def __init__(self, **kwargs):
        ...
        self.bluetooth_status_button = Button(
            ...
            # Let the applet handle rfkill before asking BlueZ to power on.
            # Calling BluetoothClient.toggle_power() directly fails when a laptop
            # hotkey or a previous session has left the radio soft-blocked.
            on_clicked=lambda *_: self.widgets.bluetooth.toggle_power(),
        )
